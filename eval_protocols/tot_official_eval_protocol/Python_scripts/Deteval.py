...
eval_cache = sys.argv[3]  #eval-dir
fid_path = sys.argv[4]  #output text file directory goes here
    ...
    for gt_id, gt in enumerate(groundtruths):
        if (gt[5] == '#') and (gt[1].shape[1] > 1):
            ...
            for det_id, detection in enumerate(detections):
                ...
                detection = list(map(int, detection[2:])) #0:img_id, 1:score
				#det_y = detection[0::2]
				#det_x = detection[1::2]
                det_x = detection[0::2]
                det_y = detection[1::2]
...
for idx in range(len(global_sigma)):
    ...
    fid = open(os.path.join(eval_cache, 'per_img_det_eval.txt'), 'a+')
...
fid = open(fid_path, 'a')
temp = ('Deteval(conf=%s) | Precision: %s, Recall: %s, F-score: %s\n' %(str(conf_thresh), str(precision), str(recall), str(f_score)))
print(temp)
fid.write(temp)
fid.close()
