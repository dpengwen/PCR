...
def snake_collator(batch):
    ...
    if 'rs_hm' in cfg['heads']:    
        rs_hm = default_collate([b['rs_hm'] for b in batch])

    batch_size = len(batch)
    ct_num = torch.max(meta['ct_num'])
    if cfg.dist_4d:
        wh = torch.zeros([batch_size, ct_num, 4], dtype=torch.float)
    else:
        wh = torch.zeros([batch_size, ct_num, 2], dtype=torch.float)
    if cfg.centerness:
        centerness = torch.zeros([batch_size, ct_num, 1], dtype=torch.float)
    if cfg.train.ct_reg:
    ...
    if ct_num != 0:
        ...
        if cfg.centerness:
            centerness[ct_01] = torch.Tensor(sum([b['centerness'] for b in batch], []))
    ...
    if cfg.centerness:
        detection.update({'centerness': centerness})
    if 'rs_hm' in cfg['heads']:    
        detection.update({'rs_hm': rs_hm})
    ...
    if cfg.poly_cls_branch:
        # neg_evolution
        neg_i_it_pys = torch.zeros([batch_size, ct_num, snake_config.poly_num, 2], dtype=torch.float)
        neg_c_it_pys = torch.zeros([batch_size, ct_num, snake_config.poly_num, 2], dtype=torch.float)
        neg_i_gt_pys = torch.zeros([batch_size, ct_num, snake_config.gt_poly_num, 2], dtype=torch.float)
        neg_c_gt_pys = torch.zeros([batch_size, ct_num, snake_config.gt_poly_num, 2], dtype=torch.float)
        if ct_num != 0:
            neg_i_it_pys[ct_01] = torch.Tensor(sum([b['neg_i_it_py'] for b in batch], []))
            neg_c_it_pys[ct_01] = torch.Tensor(sum([b['neg_c_it_py'] for b in batch], []))
            neg_i_gt_pys[ct_01] = torch.Tensor(sum([b['neg_i_gt_py'] for b in batch], []))
            neg_c_gt_pys[ct_01] = torch.Tensor(sum([b['neg_c_gt_py'] for b in batch], []))
